    ...
    <el-dialog title="编辑内容" width="50%" :append-to-body='true'>
        ...
        <el-form :model="video" label-width="120px">
            ...
            <el-form-item label="类型">
                <el-checkbox-group @click="getId(video.id)" v-model="categoryIds">
...
const categoryAdd = function (id) {
    const params = {
        categorys: categoryIds.value.jion(','),
        videoId: id.value
    }
...
const getCategoryData=function(categoryId){
    const params={
        videoId:categoryId
    }
    
    API.post("/videoCategory/getCategory", {params} ).then((res) => {
        const categories=res.data.data
        categoryIds.value = categories.map((category) => parseInt(category.categoryId));
    });
    return{
        categoryIds
    }
...
let video = ref(toRefs(Data).editData);

const categoryId=(toRefs(Data));
getCategoryData(categoryId.editData)
